...
val sub_result = store_thm ("sub_result",
  ``!g symlist.EVERY (gaw g) symlist ==> 
    ?w. RTC (derives g) symlist w /\ EVERY isTmnlSym w``,
       STRIP_TAC 
       \\ Induct_on `symlist` THEN SRW_TAC [][] 
       THENL [Q.EXISTS_TAC `[]` >> SRW_TAC [][RTC_RULES],
              FULL_SIMP_TAC (srw_ss()) [gaw]  
              \\ Q.EXISTS_TAC `w' ++ w` THEN SRW_TAC [] []  
              \\ `RTC (derives g) (h::symlist) (w' ++ w) = RTC (derives g) ([h]++symlist) (w' ++ w)` 
	            by SRW_TAC [] [] 
...
``!u v.RTC (derives g) u v ==> (u=s1++lhs'++s2) ==> rule lhs lhs' IN rules g ==> RTC (derives g) (s1++[NTS lhs]++s2) v``,
       HO_MATCH_MP_TAC RTC_STRONG_INDUCT_RIGHT1 
       \\ SRW_TAC [] [RTC_RULES] 
       THENL[METIS_TAC [res1,rtc_derives_same_append_right,rtc_derives_same_append_left,RTC_SUBSET],
...
``! u v.RTC (derives g) u v ==> (u=x++y) ==> (?x' y'. ((v=x'++y') /\ RTC (derives g) x x' /\ RTC (derives g) y y' ))``,
      ...
      \\ SRW_TAC [] [] 
      THENL[METIS_TAC [RTC_RULES,RTC_REFLEXIVE],
            `(?x''.derives g x' x'' /\ (v'=x''++y')) \/ (?y''.derives g y' y'' /\ (v'=x'++y''))` 
		by METIS_TAC [lemma2] 
            \\ METIS_TAC [RTC_RULES_RIGHT1]]
);
...
``! u v.RTC (derives g) u v ==>
      (u=x++y++z) ==> 
          (?x' y' z'. ((v=x'++y'++z') /\ RTC (derives g) x x' /\ RTC (derives g) y y' /\ RTC (derives g) z z'))``,
        HO_MATCH_MP_TAC RTC_INDUCT_RIGHT1 
        \\ SRW_TAC [] [] 
        \\ `derives g (x' ++ (y' ++ z')) v' ==> 
              (?x''.derives g x' x'' /\ (v'=x''++(y'++z'))) \/
              (?y''.derives g (y'++z') y'' /\ (v'=x'++y''))` 
		by SRW_TAC [][lemma2,listTheory.APPEND,listTheory.APPEND_ASSOC] 
        \\ `derives g (x' ++ y' ++ z') v' =  derives g (x' ++ (y' ++ z')) v'` by SRW_TAC [] [] 
        \\ RES_TAC 
        THENL[METIS_TAC [listTheory.APPEND,listTheory.APPEND_ASSOC,RTC_RULES_RIGHT1],
                 RES_TAC 
                 \\ `derives g (y' ++ z') y'' ==>
                         (?s1'.derives g y' s1' /\ (y''=s1'++z')) \/ 
                         (?s2'.derives g z' s2' /\ (y''=y'++s2'))` by METIS_TAC [lemma2] 
                 \\ RES_TAC 
		 \\ METIS_TAC [RTC_RULES_RIGHT1,listTheory.APPEND_ASSOC,listTheory.APPEND]]
);

val slemma1_4 = store_thm("slemma1_4",
``(NTS nt IN nonTerminals g) = (?rhs.rule nt rhs IN rules g \/ 
 ?l p s.rule l (p++[NTS nt]++s) IN rules g \/ (nt=startSym g))``,
       Cases_on `g` 
       \\ SRW_TAC [] [EQ_IMP_THM] 
       ...
       THENL[Cases_on `x` 
	     ...
	     \\ SRW_TAC [] [] 
	     THENL[FULL_SIMP_TAC (srw_ss()) [rule_nonterminals_def,rules_def,INSERT_DEF], METIS_TAC []],
             DISJ2_TAC 
	     \\ Q.EXISTS_TAC `rule_nonterminals (rule l (p ++ [NTS nt] ++ s'))` 
	     \\ SRW_TAC [] [] 
	     THENL[FULL_SIMP_TAC (srw_ss()) [rule_nonterminals_def,rules_def,INSERT_DEF,isNonTmnlSym_def],
                   METIS_TAC [rules_def]]
	    ]
);
